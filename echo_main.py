...
def main(args: argparse.Namespace) -> None:
    ...
    if args.echo:
        ...
        print("Start evaluation using new file format...")
        produce_echo(
            model, device=device, directory = "./wavs"
        )
...
def pad_or_trim(audio, max_len=64600):
    """
    Pad or trim the audio signal to the desired length.
    Args:
        audio: Input audio signal as a 1D numpy array.
        length: Desired length (number of samples).
    Returns:
        Processed audio signal as a numpy array.
    """
    audio_len = len(audio)
    if audio_len >= max_len:
        return audio[:max_len]
    # Repeat audio if shorter
    num_repeats = int(max_len / audio_len) + 1
    padded_audio = np.tile(audio, num_repeats)[:max_len]
    return padded_audio


def produce_echo(model, device, directory, target_length=64600, output_file="evaluation_results.txt"):
    model.eval()  # Set the model to evaluation mode

    # List all .wav files in the directory
    wav_files = [f for f in os.listdir(directory)]

    # Open output file
    with open(output_file, "w") as f_out:
        for wav_file in tqdm(wav_files, desc="Evaluating files"):
            try:
                # Full path to the audio file
                file_path = os.path.join(directory, wav_file)

                # Load and preprocess audio
                audio, _ = sf.read(file_path)  # Read the audio file
                audio = pad_or_trim(audio, target_length)  # Pad or trim the audio
                batch_x = Tensor(audio).unsqueeze(0).to(device)  # Add batch dimension and move to device

                # Make predictions
                with torch.no_grad():
                    _, batch_out = model(batch_x)
                    batch_score = (batch_out[:, 1]).item()  # Prediction score for the "positive" class

                # Write the file name and prediction score
                f_out.write(f"{wav_file} {batch_score:.4f}\n")

            except Exception as e:
                print(f"Error processing file: {wav_file} - {e}")
                f_out.write(f"{wav_file} ERROR\n")  # Mark error in output for this file
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--echo",
        action="store_true",
        help="when this flag is given, evaluate on the Echo dataset")
